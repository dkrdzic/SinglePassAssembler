...
{    
  ...
  this->instructionsInsert();
    ...
    if (!this->singleRun(inputFile)){cout<<"Error line: "<<currentLine<<endl; exit(1);}
    



if (!this->isEverythingOkWithTable()){cout<<"Error line: "<<currentLine<<endl; exit(1);}


this->resolveExternSymbols();


if (!this->equResolve()) {cout<<"Error line: "<<currentLine<<endl; exit(1);}
...
this->sortTableOfSymbols();
  ...
  { 
      if (this->fix(*it)) continue; else {cout<<"Error line: "<<currentLine<<endl;exit(1);}
...
regex commentRegex("^ *#.*$");
    ...
    if (regex_match(line, globalExternRegex) || regex_match(line, sectionRegex) || regex_match(line, byteWordRegex) || regex_match(line, skipRegex) || regex_match(line, equRegex) || regex_match(line, labelRegex) || regex_match(line, noOpInstRegex) || regex_match(line, oneOpInstJMPRegex1) 
        || regex_match(line,byteWordRegex)||regex_match(line, oneOpInstJMPRegex2) || regex_match(line, oneOpInstPushPopRegex)  || regex_match(line, twoOpInstRegex)|| regex_match(line,pcRelRegexOneOp)|| regex_match(line,pcRelRegexTwoOpsDst)||regex_match(line,pcRelRegexTwoOpsSrc)||regex_match(line,pcRelRegexTwoOpsSrcDst)|| regex_match(line,endRegex))
    {
...
this->currentLine++;
            ...
            else
            {
             ...
             regex oneOpInstRegex("(int|call|jmp|jeq|jne|jgt|push|pop)"); 
                ...
                LABEL: string token = this->tokensInLine.front();
                ...
                else if (token == ".equ")
                {
                    ...
                    if (this->label(token)){
                        ...
                        if (this->tokensInLine.empty()) continue;
                        else goto LABEL;
                    }
                    else return 0;
...
int Assembler::globalExtern(int externBool)
{
    ...
    for (int i = 0; i < size; i++)
    {
        ...
        if (externBool) symbol->externBool=1;
        else symbol->globalBool=1;
        ...
        else
        {
            ...
            }else{//drugi put ulazi u .extern ili .global
...
if (symbol->defined){cout<<"Symbol is already defined"<<endl; return 0;}
...
if (expression[0]!='-' && expression[0]!='+')expression.insert(0,1,'+');

while(i<expression.size()){


int j=0;
if (expression[i]=='-')numString.insert(0,1,'-');
else 
...
if (this->currentSection=="*UND*"){cout<<"Label must be in section"<<endl; return 0;}
...
if(symbol->defined==1){cout<<"Symbol "<<symbol->label<<" is already defined"; return 0;}
...
if(token=="*UND*"){cout<<"*UND* is reserved for section name"<<endl; return 0;}
...
if (symbol->defined && !symbol->isSectionSymbol) {cout<<"Symbol is already defined as label or equ symbol"<<endl; return 0;}
//ako je simbol vec definisan kao labela ne moze biti i kao sekcija

if (symbol->defined) {this->LC=symbol->lastLCInSection;this->PC=symbol->nextPCInSection;} //ako je opet ista sekcija nastavi sa lc gde si stao
else this->LC=0;
...
if (token=="halt") output->memory="00";
else if(token=="iret") output->memory="08";
else if (token=="ret") output->memory="10";
...
if (regex_match(dst,immedRegex)){cout<<"Imm operand cant't be destination"<<endl; return 0;}

if (regex_match(src,immedRegex) && (instruction=="xchg" || instruction=="xchgw" || instruction=="xchgb")) {cout<<"In instruction xchg src operand can't be immed value!"<<endl; return 0;}
...
if ((regex_match(dst,memdirRegex)||regex_match(src,memdirRegex)|| regex_match(src,regindRegex)|| regex_match(dst,regindRegex)||  regex_match(src,regindpomRegex)|| regex_match(dst,regindpomRegex) )&& (*it)->operandSize==-1 
     && (!regex_match(dst,regdirRegex) && !regex_match(src,regdirRegex))) {cout<<"Error: ambiguous operand size for `"<<(*it)->instruction<<"'!"<<endl; return 0;}
...
if (regex_match(dst,regdirRegex)){ if (!this->regdir((*it),dst,0, &dstSize,outputDstOperand)) return 0; }
else if (regex_match(dst,regindRegex)){ if (!this->regind((*it),src,dst,0,&dstSize,outputDstOperand)) return 0;}
else if (regex_match(dst,memdirRegex)){if (!this->memdir((*it),src,dst,0,&dstSize,outputDstOperand)) return 0; this->PC++; this->PC++;}
else if(regex_match(dst,regindpomRegex)){if (!this->regindpom((*it),src,dst,0,&dstSize,outputDstOperand,&pomDst))return 0; this->PC++; this->PC++;}
else {cout<<"Error: invalid adressing!"<<endl; return 0;}
...
if (dstSize==1 && (*itHelp)->firstByte=="??"){ itHelp++; outputFirstByte->memory=(*itHelp)->firstByte;}
else if (dstSize==2 && (*itHelp)->firstByte=="??"){itHelp++;itHelp++; outputFirstByte->memory=(*itHelp)->firstByte;}
else outputFirstByte->memory=(*itHelp)->firstByte;



if (regex_match(src,immedRegex)) { if (!this->immed((*it),src,&dstSize,outputSrcOperand)) return 0;this->PC+=dstSize;}
else if (regex_match(src,regdirRegex)){if(!this->regdir((*it),src,1,&dstSize,outputSrcOperand))return 0;}
else if (regex_match(src,regindRegex)){if (!this->regind((*it),src,dst,1,&dstSize,outputSrcOperand)) return 0; }
else if (regex_match(src,memdirRegex)){if (!this->memdir((*it),src,dst,1,&dstSize,outputSrcOperand)) return 0; this->PC++; this->PC++;}
else if(regex_match(src,regindpomRegex)){if (!this->regindpom((*it),src,dst,1,&dstSize,outputSrcOperand,&pomSrc))return 0; this->PC++; this->PC++;}
else {cout<<"Error: invalid adressing!"<<endl; return 0;}
...
if (outputSrcOperand->adressing=="memdir") this->ImDiAd(src,2,imDiAdOutputSrc,"R_X86_16_16");
else if(outputSrcOperand->adressing=="immed") {src.erase(0,1);string relocType;if (dstSize==1)relocType="R_X86_16_8";else relocType="R_X86_16_16";this->ImDiAd(src,dstSize,imDiAdOutputSrc,relocType);}
    ...
    else this->ImDiAd(pomSrc,2,imDiAdOutputSrc,"R_X86_16_16");
    }

outputDstOperand->locationCounter=this->LC;
this->LC++; 

if (outputDstOperand->adressing=="memdir")this->ImDiAd(dst,2,imDiAdOutputDst,"R_X86_16_16");
    ...
    else this->ImDiAd(pomDst,2,imDiAdOutputDst,"R_X86_16_16");
...
if (outputSrcOperand->adressing!="?")outputLine.push_back(imDiAdOutputSrc);
outputLine.push_back(outputDstOperand);
if (outputDstOperand->adressing!="?")outputLine.push_back(imDiAdOutputDst);
...
if (regex_match(operand,immedRegex)&& (*it)->instructions==POP){cout<<"POP instruction can't have immed adressing!"<<endl; return 0;}
...
if (!regex_match(operand,regdir)){cout<<"There are only 8 registers from r0 to r7!"<<endl; return 0;}
...
if (operand[3]=='l') cout<<"Value of operand is extended to 2B!"<<endl;
...
if (!regex_match(operand,regindpomRegex)){cout<<"There is no such register!"<<endl; return 0;}
...
if (operand[i+2]=='p' || operand[j]=='7'){if(!regex_match(pom,symbolRegex)){cout<<"Pcrel must have symbol as offset!"<<endl; return 0;} imDiAdOutput->pcRel=1;regNum=7;imDiAdOutput->PC=this->PC;}
else
...
else this->ImDiAd(pom,2,imDiAdOutput,"R_X86_16_16");
...
if (!regex_match(operand,regindRegex)){cout<<"Regsters are from 0-7!"<<endl; return 0;}
...
if (operand[i+2]=='p' || operand[j]=='7'){ imDiAdOutput->pcRel=1;regNum=7;imDiAdOutput->PC=this->PC;}
else
 regNum=(int) operand[j]-48;
...
if (operand[3]!='l') cout<<"Value of src operand is truncated to 1B!"<<endl;
...
if(inst->operandSize==1){//instrukcija sa b
...
if (operand[3]!='l') cout<<"Value is truncated to 1B!"<<endl;
...
else if (inst->operandSize==2){//instrukcija sa w
...
if (operand[3]=='l') cout<<"Value of src is truncated to 1B!"<<endl;
...
else if (inst->operandSize==-1){ //za instrukcije bez b i w
...
if (!regex_match(dst,regind)){cout<<"Regsters are from 0-7!"<<endl; return 0;}
...
}else if (inst->operandSize==-1){//ako je instrukcija bez b ili w znaci da je src regdir

if(src[3]!='h' && src[3]!='l') *dstSize=2;
else *dstSize=1;
...
if (!regex_match(src,regind)){cout<<"Regsters are from 0-7!"<<endl; return 0;}
...
if (!regex_match(dst,regindpomRegex)){cout<<"There is no such register!"<<endl; return 0;}

if (inst->operandSize==1) *dstSize=1;
else if (inst->operandSize==2) *dstSize=2;
else if (inst->operandSize==-1){//znaci da je src reg inace bi bila greska
...
if (!regex_match(src,regindpomRegex)){cout<<"There is no such register!"<<endl; return 0;}
...
if (imDiAdSize==1){// u slucaju kad je simbol immed adresiranja
...
if (num>255){while(hex.size()>2)hex.erase(0,1); cout<<"Value "<<help<<" truncated to "<<hex<<endl;}


outputOpDescr->locationCounter=this->LC;
outputOpDescr->section=this->currentSection;

if(num<=15) hex.insert(0,1,'0');
outputOpDescr->memory=hex;
...
if (num>65535){while(hex.size()>4)hex.erase(0,1); cout<<"Value "<<help<<" truncated to "<<hex<<endl;}

outputOpDescr->locationCounter=this->LC;

outputOpDescr->section=this->currentSection;

if(num<=15) {hex.insert(0,3,'0');}
else if(num<=255){hex.insert(0,2,'0');}
else if(num<=4095){hex.insert(0,1,'0');}
...
if (dec>255){while(operand.size()>2)operand.erase(0,1); cout<<"Value "<<help<<" truncated to "<<operand<<endl;}

if (dec<=15) operand.insert(0,1,'0');
outputOpDescr->locationCounter=this->LC;
outputOpDescr->section=this->currentSection;
...
if (dec>65535){while(operand.size()>4)operand.erase(0,1); cout<<"Value "<<help<<" truncated to "<<operand<<endl;}

if(dec<=15) {operand.insert(0,3,'0');}
else if(dec<=255){operand.insert(0,2,'0');}
else if(dec<=4095){operand.insert(0,1,'0');}
...
digit=byte;
    ...
    string help=hex;
if (digitHelp>128){while(hex.size()>2)hex.erase(0,1); cout<<"Value "<<help<<" truncated to "<<hex<<endl;}
...
if (digit>32768){cout<<"Value can't fit"<<endl; return 0;}
...
if (dec==0) {outputOpDescr->memory="00";
 this->LC++;
outputOpDescr->section=this->currentSection;
...
if (dec>32768){cout<<"Value can't fit"<<endl; return 0;}

if (dec==0) {outputOpDescr->memory="00 00"; 
this->LC++;
this->LC++;
...
string section;
int index;
ClassificationIndex(string section,int index){this->section=section; this->index=index;}
...
Symbol* symbolExp=this->findSymbol(help);
 ...
 if ( symbolExp!=0 && (symbolExp->externBool || symbolExp->equExtern)) {equExtern=1; if(!symbolExp->isEquSymbol)equExternSymbol=symbolExp->label;else equExternSymbol=symbolExp->equExternSymbol; ;sign='+';}



if(symbolExp==0){cout<<"Symbol "<<help <<" is undefined"<<endl; return 0;}

else if(symbolExp->unsolved==1){cout<<"Symbol is still unsolved "<<endl; goToAnotherSymbol=1; break;}
...
if ((*itIndex)->section==symbolExp->section)break;
else continue;
}

if (sign=='-') (*itIndex)->index+=-1;
else (*itIndex)->index+=1;
...
if((*itIndex)->index!=1 && (*itIndex)->index!=0) {cout<<"Not correct equ expression for symbol '"<<symbol->label<<"'!"<<endl; return 0;}

if((*itIndex)->index==1){ numOfOnes++;symbol->section=(*itIndex)->section;}


}

if (numOfOnes>1){cout<<"Not correct equ expression for symbol '"<<symbol->label<<"'!"<<endl; return 0;}


if (equExtern) {symbol->equExtern=1; symbol->section="*UND*"; symbol->equExternSymbol=equExternSymbol;}
...
if(!this->TNS.empty()){cout<<"There are unsolved symbols!"<<endl; return 0;}
...
if ((*it2)->locationCounter==(*itfr)->LC && (*it2)->section==(*itfr)->section)  it2=this->relocTable.erase(it2);
else continue;
...
if ((*it2)->locationCounter==(*it)->LC && (*it2)->section==(*it)->section)  break;
else continue;
...
if ((*it4)->locationCounter==(*it)->LC && (*it4)->section==(*it)->section) { findOutput=1;break;}
else continue;

}


if (findOutput==1) break;
else continue;
...
{this->relocTable.erase(it2);it2=this->relocTable.end();}


int imDiAd=symbol->offset-(*it4)->PC;


if (imDiAd==0) (*it4)->memory="00 00";
else
if (imDiAd<0){

imDiAd=-imDiAd;

if (imDiAd>32768){cout<<"Value can't fit!"<<endl; return 0;}
...
if (imDiAd>65535){cout<<"Value can't fit!"<<endl; return 0;}

if(imDiAd<=15) {hex.insert(0,3,'0');}
else if(imDiAd<=255){hex.insert(0,2,'0');}
else if(imDiAd<=4095){hex.insert(0,1,'0');}
...
else { //nije pcrel sa const simbolom ili istom sekcijom intrsukcije i simbola ili aps adresiranje
...
if (symbol->offset<=15) {hex.insert(0,1,'0'); }
   }
...
}else {cout<<"ERROR! Offset can't fit!"; return 0;}


}
else {cout<<"Symbol "<<symbol->label<<" is not defined!"; return 0;}
...
if(symbol->offset<=15) {hex.insert(0,3,'0');}
else if(symbol->offset<=255){hex.insert(0,2,'0');}
else if(symbol->offset<=4095){hex.insert(0,1,'0');}
...
if (symbol->label=="z") cout<<"HEJ5"<<endl;
...
if (dif>0){binary.insert(0,dif,'1');}
...
int k=i;
int j=i-1;
while(k>0) {binary.append(to_string(binaryNum[j]));k--;j--;}

if (i==0) binary.append("0");
...
void Assembler::printOutput()
{
    ...
    for (it1 = this->output.begin(); it1 != this->output.end(); it1++)
    {
        ...
        {
            if (currentSection=="*UND*"){currentSection=(*it2)->section;cout<<endl; cout<<"----------"<<currentSection<<"----------"<<endl;}
            if(currentSection!=(*it2)->section){
                currentSection=(*it2)->section;
                cout<<endl<<"----------"<<currentSection<<"----------"<<endl;
            }
...
cout<<"-------------------TABLE OF SYMBOLS-------------------"<<endl<<endl;

cout<<"|"<<setw(10)<<"LABEL"<<"|"<<setw(10)<<"SECTION"<<"|"<<setw(10)<<"OFFSET"<<"|"<<setw(10)<<"LOCAL?"<<"|"<<setw(10)<<"NUMBER"<<"|"<<endl;
...
if((*it)->defined==0 && (*it)->globalBool==1){cout<<"Exporting symbol "<<(*it)->label<<" that is not defined!"<<endl; return 0;}

if ((*it)->defined==0 && (*it)->externBool==0) {cout<<"Undefined symbol "<<(*it)->label<<endl; return 0;}

if ((*it)->isSectionSymbol==1 && (*it)->globalBool==1){cout<<"Section symbols are local"<<endl; return 0;}
...
if((*it)->globalBool==1 && (*it)->externBool==1){cout<<"There is already a global label with name '"<<(*it)->label<<"' defined in another file"<<endl; return 0;}
...
cout<<"-------------------RELOCATION TABLE-------------------"<<endl<<endl;

cout<<"|"<<setw(10)<<"SECTION"<<"|"<<setw(10)<<"LC"<<"|"<<setw(15)<<"TYPE"<<"|"<<setw(10)<<"VALUE"<<"|"<<endl;
...
cout<<"Outputfile can't be opened!"<<endl; exit(1);
...
outFile<<endl;
//tabela simbola
 outFile<<"-------------------TABLE OF SYMBOLS-------------------"<<endl<<endl;

outFile<<"|"<<setw(10)<<"LABEL"<<"|"<<setw(10)<<"SECTION"<<"|"<<setw(10)<<"OFFSET"<<"|"<<setw(10)<<"LOCAL?"<<"|"<<setw(10)<<"NUMBER"<<"|"<<endl;
...
//relocTable
    ...
    outFile<<"-------------------RELOCATION TABLE-------------------"<<endl<<endl;

outFile<<"|"<<setw(10)<<"SECTION"<<"|"<<setw(10)<<"LC"<<"|"<<setw(15)<<"TYPE"<<"|"<<setw(10)<<"VALUE"<<"|"<<endl;
...
outFile<<"----------------- MEMORY ---------------------"<<endl;
    ...
    for (it1 = this->output.begin(); it1 != this->output.end(); it1++)
    {
        ...
        for (it2 = it1->begin(); it2 != it1->end(); it2++)
        {
            if (currentSection=="*UND*"){currentSection=(*it2)->section;outFile<<endl; outFile<<"----------"<<currentSection<<"----------"<<endl;}
            if(currentSection!=(*it2)->section){
                currentSection=(*it2)->section;
                
                outFile<<endl<<"----------"<<currentSection<<"----------"<<endl;
            }
